...
class FragmentTransition {
    ...
    private static void addToFirstInLastOut(BackStackRecord transaction, Op op, SparseArray<FragmentContainerTransition> transitioningFragments, boolean isPop, boolean isReorderedTransaction) {
        ...
        if (fragment2 != null) {
            ...
            if (containerId != 0) {
                int command = z ? INVERSE_OPS[op2.mCmd] : op2.mCmd;
                boolean z2 = false;
                if (command != 1) {
                    if (command != 3) {
                        if (command == 4) {
                        ...
                        FragmentContainerTransition containerTransition5 = (FragmentContainerTransition) sparseArray.get(containerId);
                        if (setLastIn) {
                    ...
                    wasAdded = false;
                    FragmentContainerTransition containerTransition52 = (FragmentContainerTransition) sparseArray.get(containerId);
                    if (setLastIn) {
                    }
                    if (!isReorderedTransaction) {
                    }
                ...
                setFirstOut = false;
                wasAdded = true;
                FragmentContainerTransition containerTransition522 = (FragmentContainerTransition) sparseArray.get(containerId);
                if (setLastIn) {
                }
                if (!isReorderedTransaction) {
                }
