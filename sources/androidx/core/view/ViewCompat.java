...
public class ViewCompat {
    ...
    private static List<AccessibilityActionCompat> getActionList(View view) {
        ArrayList<AccessibilityActionCompat> actions = (ArrayList) view.getTag(R.id.tag_accessibility_actions);
        if (actions != null) {
            return actions;
        }
    ...
    public static void addOnUnhandledKeyEventListener(View v, final OnUnhandledKeyEventListenerCompat listener) {
        if (VERSION.SDK_INT >= 28) {
            Map map = (Map) v.getTag(R.id.tag_unhandled_key_listeners);
            if (map == null) {
                map = new ArrayMap();
                v.setTag(R.id.tag_unhandled_key_listeners, map);
            }
    ...
    public static void removeOnUnhandledKeyEventListener(View v, OnUnhandledKeyEventListenerCompat listener) {
        if (VERSION.SDK_INT >= 28) {
            Map<OnUnhandledKeyEventListenerCompat, OnUnhandledKeyEventListener> viewListeners = (Map) v.getTag(R.id.tag_unhandled_key_listeners);
            if (viewListeners != null) {
