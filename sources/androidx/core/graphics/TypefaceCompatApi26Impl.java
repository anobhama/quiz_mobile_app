...
public class TypefaceCompatApi26Impl extends TypefaceCompatApi21Impl {
    ...
    public Typeface createFromFontInfo(Context context, CancellationSignal cancellationSignal, FontInfo[] fonts, int style) {
        ...
        if (!isFontFamilyPrivateAPIAvailable()) {
            FontInfo bestFont = findBestInfo(fontInfoArr, i2);
            try {
                ParcelFileDescriptor pfd = context.getContentResolver().openFileDescriptor(bestFont.getUri(), "r", cancellationSignal2);
                if (pfd == null) {
                    if (pfd != null) {
                        pfd.close();
                    }
